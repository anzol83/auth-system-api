...
userRouter.post('/login', async(req, res) => {
  try {
    ...
    const sessionStorage = await createSession({ userEmail: user.email, token: jwt.accessJWT })
...
userRouter.get('/accessjwt', refreshAuth, async(req, res) => {
  try {
    if(req.userInfo){
      ...
      // add to session storage
      const sessionStorage = await createSession({ userEmail: email, token: accessJWT })
